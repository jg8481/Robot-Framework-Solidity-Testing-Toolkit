...
[The capabilities of the Solidity programming language and the Ethereum platform in general are vast and growing every day](https://soliditylang.org/). The very simple OpenZeppelin ERC20 `TestToken` and other smart contract examples included in this toolkit are tiny in comparison to what is being used today in Web3 and DeFi production environments. I have included small tests and basic RPA automation scripts that work for Ethereum, Fantom, Avalanche, Polygon, but can be adapted to cover many others as well by simply adjusting minor `hardhat` CLI options and altering the provided config files. In the future I plan to expand this toolkit into the following areas.
