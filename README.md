...
[The capabilities of the Solidity programming language and the Ethereum platform in general are vast and growing every day](https://soliditylang.org/). The limited ERC20 `TestToken` and other smart contract examples included in this toolkit are small in comparison to what is being used today in Web3 and DeFi. I have included small tests and RPA automation building blocks that work for Ethereum, Fantom, Avalanche, Polygon, but it can cover many others as well by simply adjusting minor `hardhat` CLI options and altering the provided config files. In the future I plan to expand this toolkit into the following areas.
